...
PredPrey::PredPrey(int num_of_predators, int num_of_prey, int num_teams_predator, int num_teams_prey, vector <double> prey_move_probability)
{
   ...
   cout << "   inputSize per agent :  " << " Predator:: "<< inputSize << " Prey:: " << inputSize_prey << "  Predator Combiner:: "<< inputSize_pred_combiner<<"  Prey Combiner::  "<<inputSize_prey_combiner<<endl;
   cout << "   outputSize per agent:  " << " Predator:: "<< outputSize << " Prey:: " << outputSize_prey << "  Predator Combiner:: "<< outputSize_pred_combiner<<"  Prey Combiner::  "<<outputSize_prey_combiner<<endl;
...
vector < vector < vector<double> > > PredPrey::evalNet (vector <vector<Network*> >& team, vector <vector<Network*> >& team_prey, int generation) //Added number of prey to arguments ******PADMINI
{
...
// ave_initial_dist /= num_of_predators;
 ...
 while (steps++ < maxSteps && temp_break_while == false) {
  ...
  for (p=0; p< num_teams_predator; p++) {
          ...
          for (i=0; i<num_of_predators; i++) {
                  ...
                  if (COMBINE == 1) {
                          team[p][(inputSize/2) * num_of_predators  + i]->activate(temp_output, temp_combiner_output, num_of_prey, num_of_predators, num_teams_prey, num_teams_predator, inputSize_pred_combiner);
                  } 
                  else {
 ...
 for (int p = 0; p < num_teams_predator; p++) {
          ...
          for (int i = 0; i < num_of_predators; i++) {
                  ...
                  if (num_teams_prey == 1) {
                          temp_individual_fitness.push_back(num_of_prey_caught_individually[p][i] * reward_prey_team[0] + num_of_prey_caught[p] * reward_prey_team[0]); // This is individual fitness for Competing agents 
                  }
                  else if (num_teams_prey == 2) {
                          temp_individual_fitness.push_back(num_of_prey_caught_individually[p][i] * reward_prey_team[0] + num_of_prey_caught[p] * reward_prey_team[1]); // This is individual fitness for Competing agents 
                  }
                  else {
...
if (generation > previous_generation) {
        ...
        cout << " Generation Number :: " << previous_generation << " Overall Best Number of Zebra Caught :  " << count_overall_best_zebra_caught<< endl;
        cout << " Generation Number :: " << previous_generation << " Generation Best Number of Zebra Caught :  " << count_generation_best_zebra_caught<< endl;
        cout << " Generation Number :: " << previous_generation << " Generation Average Number of Zebra Caught :  " <<count_generation_zebra_caught/TOTAL_EVALUATIONS << endl;
        cout << " Generation Number :: " << previous_generation << " Overall Best Number of Mice Caught :  " << count_overall_best_mice_caught<< endl;
        cout << " Generation Number :: " << previous_generation << " Generation Best Number of Mice Caught :  " << count_generation_best_mice_caught<< endl;
        cout << " Generation Number :: " << previous_generation << " Generation Average Number of Mice Caught :  " <<count_generation_mice_caught/TOTAL_EVALUATIONS << endl;
        ...
        count_generation_mice_caught = num_of_prey_caught_individually[0][0] + num_of_prey_caught_individually[0][1] + num_of_prey_caught_individually[0][2];
        previous_generation = generation;
}
else {
        count_generation_zebra_caught = count_generation_zebra_caught + num_of_prey_caught[0];
        count_generation_mice_caught = count_generation_mice_caught + num_of_prey_caught_individually[0][0] + num_of_prey_caught_individually[0][1] + num_of_prey_caught_individually[0][2];
...
if (count_trials == 0) {
        ...
        count_mice_caught_6 = num_of_prey_caught_individually[0][0] + num_of_prey_caught_individually[0][1] + num_of_prey_caught_individually[0][2];
}
else {
        count_zebra_caught_6 = count_zebra_caught_6 + num_of_prey_caught[0];
        count_mice_caught_6 = count_mice_caught_6 + num_of_prey_caught_individually[0][0] + num_of_prey_caught_individually[0][1] + num_of_prey_caught_individually[0][2];
}
count_trials++ ;
...
vector < vector < double > > PredPrey::testNet (vector <vector<Network*> >& team, vector <vector<Network*> >& team_prey, int trials) {
...
void PredPrey::init (bool preyRandom, bool predsRandom, int generation)
{
                                ...
                                for(j = 0; j < num_of_predators; j++) { 
	    	                        ...
	    	                        if ((abs(y_dist)) > (MAP_HEIGHT/2)) {
	    	                        	...
	    	                        	if (temp > 0)
                                            ...
                                            team_prey[p][count]->activate(temp_input_prey, temp_output_prey, num_of_predators, num_of_prey, num_teams_predator, num_teams_prey, inputSize_prey_combiner);
                                            count++;
                                            for(int k = 0; k < NUM_OUTPUTS_PREY; k++) {                   
...
/**********************************************************************************************/
void PredPrey::setupInput_complex_predator(int num_of_prey, vector< vector<double> >& output, vector <vector<Network*> >& team)
{
	//Changed these to vectors to store the x- and y-distances of the predators from the multiple prey ******PADMINI
	int x_dist;
	int y_dist;
                                ...
                                for(j = 0; j < num_of_prey; j++) { 
	    	                        ...
	    	                        if ((abs(y_dist)) > (MAP_HEIGHT/2)) {
	    	                        	...
	    	                        	if (temp > 0)
                                            ...
                                            //temp_input.push_back(0);  //Energy difference with Prey is zero
                                            //temp_input.push_back(0);  //Dummy input
                                            team[p][count]->activate(temp_input, temp_output, num_of_prey, num_of_predators, num_teams_prey, num_teams_predator, inputSize_pred_combiner);
                                            count++;
                                            for(int k = 0; k < NUM_OUTPUTS; k++) {                   
                                                    output_per_team.push_back(temp_output[k]);
                                            }
...
void PredPrey::performPredAction_complex(int pred_team, int pred, const vector<double>& output_single_predator)
{ 	
	int predAction=0;
	...
	//if they've caught all the prey. Also, the same predator needn't have caught all the prey. ******PADMINI
        ...
        for (int p = 0; p < num_teams_prey; p++) {        
	        for(i = 0; i < num_of_prey; i++) {
	        	if(prey_caught[p][i] == false && pred_killed[pred_team][pred] == false && pred_x[pred_team][pred] == prey_x[p][i] && pred_y[pred_team][pred] == prey_y[p][i]) {
        ...
        for (int p = 0; p < num_teams_predator; p++) {        
	        for(i = 0; i < num_of_predators && p!= pred_team; i++) {
	        	if(pred_killed[p][i] == false && pred_killed[pred_team][pred] == false && pred_x[pred_team][pred] == pred_x[p][i] && pred_y[pred_team][pred] == pred_y[p][i]) {
...
void PredPrey::performPreyAction_complex (int prey_team, int prey, const vector<double>& output_single_prey) { //Added the argument int prey to check which prey is being processed ******PADMINI
	...
	int old_prey_y = prey_y[prey_team][prey];
        ...
        /********Commenting out for now since prey is evolving**********************/
                   for (int p = 0; p < num_teams_predator; p++) {        
			for (i=0; i<num_of_predators; i++) {
				dist = calc_dist (pred_x[p][i], pred_y[p][i], prey_x[prey_team][prey], prey_y[prey_team][prey]);
				if (dist < nearestDist) {
					nearestDist = dist;
					nearestPred = i;
                                        nearestPred_team = p;
...
void PredPrey::nextTask () {
  ...
  cout << endl
       << "Next task. prey_move_factor: " << prey_move_factor
       << " , prey_flee_factor: " << prey_flee_factor << endl
       << endl;
...
void PredPrey::simplifyTask () {  
  ...
  cout << endl
       << "Simplify task. prey_move_factor: " << prey_move_factor
       << " , prey_flee_factor: " << prey_flee_factor << endl
       << endl;
