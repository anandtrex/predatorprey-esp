...
PredPrey::PredPrey(int num_of_predators, int num_of_prey,
	...
	inputSize = (num_of_prey * num_teams_prey
			+ num_of_predators * (num_teams_predator - 1)) * 2; //Number of inputs for all the networks in a single predator
	inputSize_prey = (num_of_predators * num_teams_predator
			+ num_of_prey * (num_teams_prey - 1)) * 2; //Number of inputs for all the networks in a single prey
	...
	if (prey_messaging == true) {
		cout
				<< " ERROR:: PREY MESSAGING SHOULD BE FALSE SINCE PREY DO NOT EVOLVE "
				<< endl;
		outputSize_prey_combiner = outputSize_prey_combiner + 1; //Extra combiner output for prey messaging
	}

	cout << "   inputSize per agent :  " << " Predator:: " << inputSize
			<< " Prey:: " << inputSize_prey << "  Predator Combiner:: "
			<< inputSize_pred_combiner << "  Prey Combiner::  "
			<< inputSize_prey_combiner << endl;
	cout << "   outputSize per agent:  " << " Predator:: " << outputSize
			<< " Prey:: " << outputSize_prey << "  Predator Combiner:: "
			<< outputSize_pred_combiner << "  Prey Combiner::  "
			<< outputSize_prey_combiner << endl;
...
vector<vector<vector<double> > > PredPrey::evalNet(
		vector<vector<Network*> >& team, vector<vector<Network*> >& team_prey,
	...
	if (SHOW) {
		...
		if ((fptr = fopen("pred_prey_location.txt", "at")) == NULL) {
			cout << endl << "Error - cannot open " << "pred_prey_location.txt"
					<< endl;
			exit(1);
		}
...
// ave_initial_dist /= num_of_predators;
	...
	while (steps++ < maxSteps && temp_break_while == false) {
		...
		IS_PREY = true;
		vector<double> temp_output_prey, temp_combiner_output_prey(
			...
			for (i = 0; i < num_of_prey; i++) {
				...
				if (prey_caught[p][i] == true) {
					...
					if (drand48() < prey_move_probability[p]) {
						performPreyAction_complex(p, i,
								temp_combiner_output_prey);
					}
		...
		/************************************PREDATOR************************************************************/
		IS_PREY = false;
		vector<double> temp_output, temp_combiner_output(
				NUM_OUTPUT_PRED_COMBINER);
		IS_COMBINER_NW = 0;
			...
			for (i = 0; i < num_of_predators; i++) {
				...
				for (j = 0; j < inputSize_pred_combiner; j++) {
					temp_output.push_back(
							output[p][inputSize_pred_combiner * i + j]);
				}
				if (COMBINE == 1) {
					team[p][(inputSize / 2) * num_of_predators + i]->activate(
							temp_output, temp_combiner_output, num_of_prey,
				...
				if (pred_messaging == true) {
					temp_messages.push_back(
							temp_combiner_output[NUM_OUTPUT_PRED_COMBINER - 1]); //messages
		...
		for (int i = 0; i < num_of_predators; i++) {
			...
			if (num_teams_prey == 1) {
				temp_individual_fitness.push_back(
						num_of_prey_caught_individually[p][i]
								* reward_prey_team[0]
				...
				cout << " ERROR: Too many prey teams. Not supported now"
						<< endl;
				exit(0);
			}
		}
...
//Overall Best, Generation Best and Generation Average
	if (generation > previous_generation) {
		if (count_generation_best_zebra_caught
				> count_overall_best_zebra_caught) {
			count_overall_best_zebra_caught =
					count_generation_best_zebra_caught;
		}
		if (count_generation_best_mice_caught
				> count_overall_best_mice_caught) {
			count_overall_best_mice_caught = count_generation_best_mice_caught;
		}
		cout << " Generation Number :: " << previous_generation
				<< " Overall Best Number of Zebra Caught :  "
				<< count_overall_best_zebra_caught << endl;
		cout << " Generation Number :: " << previous_generation
		...
		cout << " Generation Number :: " << previous_generation
				<< " Overall Best Number of Mice Caught :  "
				<< count_overall_best_mice_caught << endl;
		cout << " Generation Number :: " << previous_generation
				<< " Generation Best Number of Mice Caught :  "
				<< count_generation_best_mice_caught << endl;
		cout << " Generation Number :: " << previous_generation
		...
		count_generation_mice_caught = num_of_prey_caught_individually[0][0]
				+ num_of_prey_caught_individually[0][1]
				+ num_of_prey_caught_individually[0][2];
		previous_generation = generation;
	} else {
		count_generation_zebra_caught = count_generation_zebra_caught
				+ num_of_prey_caught[0];
		count_generation_mice_caught = count_generation_mice_caught
				+ num_of_prey_caught_individually[0][0]
				+ num_of_prey_caught_individually[0][1]
				+ num_of_prey_caught_individually[0][2];

	}
	if (count_trials == 0) {
		if ((count_zebra_caught_6 / EVALTRIALS)
				> count_generation_best_zebra_caught) {
			count_generation_best_zebra_caught = count_zebra_caught_6
					/ EVALTRIALS;
		}
		if ((count_mice_caught_6 / EVALTRIALS)
				> count_generation_best_mice_caught) {
			count_generation_best_mice_caught = count_mice_caught_6
					/ EVALTRIALS;
		}
		...
		count_mice_caught_6 = count_mice_caught_6
				+ num_of_prey_caught_individually[0][0]
				+ num_of_prey_caught_individually[0][1]
				+ num_of_prey_caught_individually[0][2];
	}
...
// }
	...
	for (int p = 0; p < num_teams_prey; p++) {
		...
		for (int i = 0; i < num_of_prey; i++) {
			temp_individual_fitness.push_back(
					(double) prey_lifetime[p][i] / maxSteps * 100); // This is individual fitness for Competing agents
		}
		temp_team_fitness.push_back(temp_individual_fitness);
	...
	for (int i = 0; i < trials; i++) {
		...
		for (int p = 0; p < num_teams_predator; p++) {
			...
			result[p][0] = result[p][0]
					+ ((double) num_of_prey_caught[p]) / trials;
			result[p][1] = result[p][1]
					+ ((double) num_of_pred_kills[p]) / trials;
		}
	}
...
void PredPrey::init(bool preyRandom, bool predsRandom, int generation) {
	...
	for (int p = 0; p < num_teams_predator; p++) {
		...
		num_of_prey_caught_individually.push_back(
				temp_num_of_prey_caught_individually);
		temp_killed.clear();
...
//}
/**********************************************************************************************/
void PredPrey::setupInput_complex_prey(int num_of_predators,
		vector<vector<double> >& output_prey,
		...
		for (i = 0; i < num_of_prey && !prey_caught[p][i]; i++) {
			for (q = 0; q < num_teams_predator; q++) {
				for (j = 0; j < num_of_predators; j++) {
					...
					team_prey[p][count]->activate(temp_input_prey,
							temp_output_prey, num_of_predators, num_of_prey,
...
/**********************************************************************************************/
void PredPrey::setupInput_complex_predator(int num_of_prey,
		vector<vector<double> >& output, vector<vector<Network*> >& team) {
	//Changed these to vectors to store the x- and y-distances of the predators from the multiple prey ******PADMINI
	int x_dist;
	int y_dist;
	int energy_diff;
...
void PredPrey::performPredAction_complex(int pred_team, int pred,
	...
	for (int p = 0; p < num_teams_prey; p++) {
		for (i = 0; i < num_of_prey; i++) {
			if (prey_caught[p][i] == false
					&& pred_killed[pred_team][pred] == false
				...
				pred_energy[pred_team][pred] = pred_energy[pred_team][pred]
						+ 100;
				if (p == 1
						|| (if_shared_fitness_predator && (num_teams_prey == 1))) { //Adding override for fitness sharing experiment
...
void PredPrey::performPreyAction_complex(int prey_team, int prey,
		const vector<double>& output_single_prey) { //Added the argument int prey to check which prey is being processed ******PADMINI
	...
	if (prey_caught[prey_team][prey]) {
		...
		for (int p = 0; p < num_teams_predator; p++) {
			for (i = 0; i < num_of_predators; i++) {
				dist = calc_dist(pred_x[p][i], pred_y[p][i],
						prey_x[prey_team][prey], prey_y[prey_team][prey]);
				if (dist < nearestDist) {
					nearestDist = dist;
					nearestPred = i;
					nearestPred_team = p;
		...
		x_dist = pred_x[nearestPred_team][nearestPred]
		...
		y_dist = pred_y[nearestPred_team][nearestPred]
...
void PredPrey::nextTask() {
	...
	cout << endl << "Next task. prey_move_factor: " << prey_move_factor
		...
		if (prey_communication == true) { //Aditya : March 03 - Communication
			for (int i = 0; i < NUM_OUTPUTS_PREY; i++) {
				...
				for (int j = 0; j < num_of_predators + num_of_prey - 1; j++) {
					temp_sum_outputs[i] = temp_sum_outputs[i]
							+ vec[NUM_OUTPUTS_PREY * j + i];
				}
			}
		} else {
			for (int i = 0; i < NUM_OUTPUTS_PREY; i++) {
