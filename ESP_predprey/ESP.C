...
struct maximize_fit : public binary_function<neuron*, neuron*, bool> {
  bool operator()(neuron *x, neuron *y) { return x->fitness > y->fitness; }
};
...
{ 
  ...
  if(!IS_PREY)   {
          for (; i < numBreed; ++i) {
                  crossover(pop[i]->weight, Esp::hall_of_fame_pred[drand48()*(Esp::hall_of_fame_pred.size()-1)][pred_or_prey_team][pred_or_prey_number]->pop[pop_number]->weight, 
                                  pop[numNeurons-(1+i*2)]->weight, 
                                  pop[numNeurons-(2+i*2)]->weight);
          }
  } 
          ...
          for (; i < numBreed; ++i) {
                  crossover(pop[i]->weight, Esp::hall_of_fame_prey[drand48()*(Esp::hall_of_fame_prey.size()-1)][pred_or_prey_team][pred_or_prey_number]->pop[pop_number]->weight, 
                                  pop[numNeurons-(1+i*2)]->weight, 
                                  pop[numNeurons-(2+i*2)]->weight);
          }
  }
...
//----------------------------------------------------------------------
// 1-point crossover
void subPop::crossover(const vector<double> &parent1, 
...
Esp::Esp(int num_of_predators, int num_of_prey, int num_teams_predator, int num_teams_prey, int nPops, int nSize, Environment &e, int netTp = FF)
  : Envt(e) 
{
  generation = 0;          // start at generation 0

  total_predator_networks_per_team = num_of_predators * (num_of_prey*num_teams_prey + num_of_predators*(num_teams_predator-1))  + COMBINE * num_of_predators;  //Total Predator Networks in a team

  total_prey_networks_per_team  = num_of_prey * (num_of_predators*num_teams_predator) + COMBINE * num_of_prey; //Total Prey Networks in a team

  if (pred_communication == true) {
          total_predator_networks_per_team = total_predator_networks_per_team + num_of_predators * (num_of_predators-1) ; //Extra NNs for predator direct communication
  }

  if (prey_communication == true) {
          total_prey_networks_per_team = total_prey_networks_per_team + num_of_prey * (num_of_prey-1) ; //Extra NNs for prey direct communication
  }

  if(pred_messaging == true) {
          total_predator_networks_per_team = total_predator_networks_per_team + num_of_predators * (num_of_predators-1); //Extra NNs for predator messaging  
  }
  ...
  for (int j=0; j< num_teams_predator; j++) {
          ...
          IS_COMBINER_NW = 1;
          for (int i=total_predator_networks_per_team - COMBINE * num_of_predators; i<total_predator_networks_per_team; i++) {
                  temp_overall_best_teams.push_back(genNetType(netType, numPops[i]));   // generate new net 
                  temp_overall_best_teams[i]->create();               // create net            
          } 
  ...
  for (int j=0; j< num_teams_prey; j++) {
          ...
          IS_COMBINER_NW = 1;
          for (int i=total_prey_networks_per_team - COMBINE * num_of_prey; i<total_prey_networks_per_team; i++) {
                  temp_overall_best_teams_prey.push_back(genNetType(netType, numPops[0]));   // generate new net 
                  temp_overall_best_teams_prey[i]->create();               // create net            
          } 
...
Esp::Esp(char* fname, int num_of_predators, int num_of_prey, int num_teams_predator, int num_teams_prey, int nPops, int nSize, Environment &e, int netTp = FF) :  Envt(e) 

{
      ...
      for (int j=0; j<numPops[0]; j++) 
        for (int k=0; k<total_hidden_neuron_populations_prey[p][i][j]->numNeurons; k++)
          for (int m=0; m<GENE_SIZE_PREY_NW; m++) {
            fread (&tempDouble, sizeof(double), 1, fptr);
            total_hidden_neuron_populations_prey[p][i][j]->pop[k]->weight[m] = tempDouble;   // weights
          }
  ...
  cout << "Loading from file : each predator is a separate neural net" << endl
       << "Total Number of Predator networks: " << total_predator_networks_per_team <<endl<< "Total Number of Prey Networks:" << total_prey_networks_per_team<<endl
       << "Initial total subpops: " << total_predator_networks_per_team*nPops << endl
       << "Incremental learning: " << INCREMENTAL_LEARNING << endl
       << "Burst mutation: " << BURST_MUTATE << endl
       << "EvalTrials: " << EVALTRIALS << endl
       << "STAG: " << STAG << endl << endl;
...
void Esp::setupNetDimensions(int num_of_predators, int num_of_prey)
{
  switch (netType){
  case FF:
    GENE_SIZE_PRED_NW = (NUM_INPUTS*num_of_predators)/(total_predator_networks_per_team - (COMBINE * num_of_predators)) + NUM_OUTPUTS; // Gene Size per hidden neuron (excepting combiner)  
    GENE_SIZE_PREY_NW = (NUM_INPUTS_PREY*num_of_prey)/(total_prey_networks_per_team - (COMBINE * num_of_prey)) + NUM_OUTPUTS_PREY; 
    ...
    cout << "Gene size per hidden neuron of non-combiner networks: "<< " predator:: "<<GENE_SIZE_PRED_NW<<"  prey:: "<<GENE_SIZE_PREY_NW<< endl;
    cout << "Gene size per hidden neuron of combiner networks: "<< " predator:: "<<GENE_SIZE_PRED_COMBINER_NW<<"  prey:: "<<GENE_SIZE_PREY_COMBINER_NW<< endl;
    cout << "total networks in a team  "<< " predators:: "<<total_predator_networks_per_team <<"  prey:: "<<total_prey_networks_per_team <<endl;
    cout << "FEED FORWARD\n"; break;
...
void Esp::evolve(int cycles, int num_of_predators, int num_of_prey, int num_teams_predator, int num_teams_prey)
...
Network* Esp::genNetType(int nt, int nPops)
{
	switch (nt) {
		case FF:
			return( new FeedForwardNetwork(nPops) ); break;
		/*  case FR :
...
void Esp::performEval(int num_of_predators, int num_of_prey, int num_teams_predator, int num_teams_prey)
{
  ...
  for(int i = 0; i < numTrials; ++i) {
          ...
          for (int p = 0; p < num_teams_predator; p++) {
                for (int j=0; j<total_predator_networks_per_team; j++) { 
                  for (int k=0; k<numPops[j]; k++) {
                    current_teams[p][j]->setNeuron( total_hidden_neuron_populations[p][j][k]->selectNeuron(), k );
                  }
          ...
          for (int p = 0; p < num_teams_prey; p++) {
                for (int j=0; j<total_prey_networks_per_team; j++) { 
                  for (int k=0; k<numPops[0]; k++) {
                    current_teams_prey[p][j]->setNeuron( total_hidden_neuron_populations_prey[p][j][k]->selectNeuron(), k );
                  }
          ...
          for ( int p=0; p<EVALTRIALS; p++) {
                ...
                for (int q=0; q<num_teams_predator; q++) { 
                      for (int r=0; r<num_of_predators; r++) {//Individual predator fitness is used in case of competing predators
                              predator_prey_teams_fitness[0][q][r] = predator_prey_teams_fitness[0][q][r] + teams_fitness[0][q][r];
                      }
                }
                for (int q=0; q<num_teams_prey; q++) { 
                      for (int r=0; r<num_of_prey; r++) {              
                              predator_prey_teams_fitness[1][q][r] = predator_prey_teams_fitness[1][q][r] + teams_fitness[1][q][r];
                      }
                }
          }
          ...
          for (int q=0; q<num_teams_predator; q++) {
                 ...
                 for (int r=0; r<num_of_predators; r++) {              
                         ...
                         average_predator_prey_teams_fitness[0][q][r] += predator_prey_teams_fitness[0][q][r];

                         for (int j=0; j<(total_predator_networks_per_team/num_of_predators -1); j++) {
                           current_teams[q][r*(total_predator_networks_per_team/num_of_predators -1) + j]->fitness = predator_prey_teams_fitness[0][q][r]; // distribute each predator's fitness to constituent networks
                           current_teams[q][r*(total_predator_networks_per_team/num_of_predators -1) + j]->addFitness();  // add network fitness to its neurons
                         }

                         //Following two lines are for the combiner network
                         current_teams[q][(total_predator_networks_per_team - num_of_predators) + r]->fitness = predator_prey_teams_fitness[0][q][r]; // distribute each predator's fitness to constituent networks
                 ...
                 for (int r=0; r<num_of_predators; r++) {
                         current_teams_sum_individual_fitness[q] = current_teams_sum_individual_fitness[q] + current_teams[q][r*(total_predator_networks_per_team/num_of_predators -1)] -> fitness;
                 }
                 ...
                 if(current_teams_sum_individual_fitness[q] > generation_best_teams_sum_individual_fitness[q]) {
                         //cout << " Replacing Best Team " <<endl;
                         generation_best_teams_sum_individual_fitness[q] = current_teams_sum_individual_fitness[q];
                         for (int j=0; j<total_predator_networks_per_team; j++) {
          ...
          for (int q=0; q<num_teams_prey; q++)  {
                 ...
                 for (int r=0; r<num_of_prey; r++) {              
                         ...
                         average_predator_prey_teams_fitness[1][q][r] += predator_prey_teams_fitness[1][q][r];

                         for (int j=0; j<(total_prey_networks_per_team/num_of_prey -1); j++) {
                           current_teams_prey[q][r*(total_prey_networks_per_team/num_of_prey -1) + j]->fitness = predator_prey_teams_fitness[1][q][r]; // distribute each prey's fitness to constituent networks
                           current_teams_prey[q][r*(total_prey_networks_per_team/num_of_prey -1) + j]->addFitness();  // add network fitness to its neurons
                         }

                         //Following two lines are for the combiner network
                         current_teams_prey[q][(total_prey_networks_per_team - num_of_prey) + r]->fitness = predator_prey_teams_fitness[1][q][r]; // distribute each prey's fitness to constituent networks
                 ...
                 //case of competition. However, all networks in a individual have same fitness
                 for (int r=0; r<num_of_prey; r++) {
                         current_teams_prey_sum_individual_fitness[q] = current_teams_prey_sum_individual_fitness[q] + current_teams_prey[q][r*(total_prey_networks_per_team/num_of_prey -1)] -> fitness;
                 }
                 current_teams_prey_sum_individual_fitness[q] /= num_of_prey;//Averaging over team

                 // if team outperforms bestTeam, set bestTeam = team  (Note: all networks in a individual have same fitness)
                 if(current_teams_prey_sum_individual_fitness[q] > generation_best_teams_prey_sum_individual_fitness[q]) {
                         generation_best_teams_prey_sum_individual_fitness[q] = current_teams_prey_sum_individual_fitness[q];
                         for (int j=0; j<total_prey_networks_per_team; j++) {
  ...
  for (int q=0; q<num_teams_predator; q++) {
        ...
        if(generation_best_teams_sum_individual_fitness[q] >= overall_best_teams_sum_individual_fitness[q]) { 
                overall_best_teams_sum_individual_fitness[q] = generation_best_teams_sum_individual_fitness[q];
                for (int j=0; j<total_predator_networks_per_team; j++) {
  ...
  for (int q=0; q<num_teams_prey; q++) {
        ...
        if(generation_best_teams_prey_sum_individual_fitness[q] >= overall_best_teams_prey_sum_individual_fitness[q]) {
                overall_best_teams_prey_sum_individual_fitness[q] = generation_best_teams_prey_sum_individual_fitness[q];
                for (int j=0; j<total_prey_networks_per_team; j++) {
  ...
  for (int q=0; q<num_teams_predator; q++) {
          for (int r=0; r<num_of_predators; r++) {        
              cout << "generation " << generation << ":\tPredator Team number "<< q << ":\tPredator Number "<< r <<"  Generation Best fitness:: "<<generation_best_teams[q][r*(total_predator_networks_per_team/num_of_predators -1)]-> fitness 
                      << ",  Overall Best fitness:: " << overall_best_teams[q][r*(total_predator_networks_per_team/num_of_predators -1)]->fitness   
                      << ", Generation Average fitness  " << average_predator_prey_teams_fitness[0][q][r]/numTrials
                      << "\n";
          }
  }
 ...
 //********END Commenting below because prey does not evolve****************
  ...
  for (int q=0; q<num_teams_predator; q++) {
      cout << "\t\tGeneration Best Predator team number " << q << " caught " << testNet_result[q][0] << " prey and killed  ";
  ...
  for (int q=0; q<num_teams_predator; q++) {
      cout << "\t\tOverall Best Predator team number " << q << " caught " << testNet_result[q][0] << " prey and killed  ";
      cout << testNet_result[q][1]<< " predators on average over 20 trials " ;
      cout <<endl;
  }
  cout <<endl;
  ...
  for (int q=0; q<num_teams_predator; q++) {
          for (int j=0; j<total_predator_networks_per_team; j++)
                  for (int k=0; k<numPops[j]; k++)
                          current_teams[q][j]->setNeuron( total_hidden_neuron_populations[q][j][k]->selectNeuron(), k );
  }
  // form a random net for predator teams, and test it for 20 trials
  for (int q=0; q<num_teams_prey; q++) {
          for (int j=0; j<total_prey_networks_per_team; j++)
                  for (int k=0; k<numPops[0]; k++)
                          current_teams_prey[q][j]->setNeuron( total_hidden_neuron_populations_prey[q][j][k]->selectNeuron(), k );
  }

  testNet_result = Envt.testNet (current_teams, current_teams_prey, 20); 

  for (int q=0; q<num_teams_predator; q++) {
      cout << "\t\tTypical Predator team number " << q << " caught " << testNet_result[q][0] << " prey and killed  ";
      cout << testNet_result[q][1]<< " predators on average over 20 trials " ;
      cout <<endl;
  }
  cout <<endl;
  ...
  if (if_display) {
    ...
    for ( int p=0; p<SIMU_TRIALS; p++) {
                ...
                teams_fitness =  Envt.evalNet(generation_best_teams, generation_best_teams_prey, generation);
                cout << endl<< " Simulation Trial Number:: " <<p<<endl;
                for (int q=0; q<num_teams_predator; q++) {
                      for (int r=0; r<num_of_predators; r++) {
                              predator_prey_teams_fitness[0][q][r] = predator_prey_teams_fitness[0][q][r] + teams_fitness[0][q][r];
                              cout << " Generation Best Predator Team number " << q << "  Predator Number "<< r << "  fitness:: "<<teams_fitness[0][q][r]<<endl;
                      }
                }
                for (int q=0; q<num_teams_prey; q++) { 
                      for (int r=0; r<num_of_prey; r++) {              
                              predator_prey_teams_fitness[1][q][r] = predator_prey_teams_fitness[1][q][r] + teams_fitness[1][q][r];
                              cout << " Generation Best Prey Team number " << q << "  Prey Number "<< r << "  fitness:: "<<teams_fitness[1][q][r]<<endl;
                      }
                }
    }
    ...
    for (int q=0; q<num_teams_predator; q++) { 
           for (int r=0; r<num_of_predators; r++) {
                   cout << "Average fitness for Predator Team Number " << q<< " Predator Number "<< r<< ":: "<< predator_prey_teams_fitness[0][q][r]/SIMU_TRIALS<< endl;
           }
    }
    for (int q=0; q<num_teams_prey; q++) { 
           for (int r=0; r<num_of_prey; r++) {
                   cout << "Average fitness for Prey Team Number " << q<< " Prey Number "<< r<< ":: "<< predator_prey_teams_fitness[1][q][r]/SIMU_TRIALS<< endl;
           }
    }
    SHOW=0;
  }
...
void Esp::save (char* fname, int num_of_predators, int num_of_prey, int num_teams_predator, int num_teams_prey)
{
  FILE* fptr;
      ...
      for (int j=0; j<numPops[i]; j++)
        for (int k=0; k<total_hidden_neuron_populations[p][i][j]->numNeurons; k++) 
          for (int m=0; m<GENE_SIZE_PRED_NW; m++) {               // weights
            fwrite (&(total_hidden_neuron_populations[p][i][j]->pop[k]->weight[m]),sizeof(double),1,fptr);     
          }
      ...
      for (int j=0; j<numPops[0]; j++)
        for (int k=0; k<total_hidden_neuron_populations_prey[p][i][j]->numNeurons; k++) 
          for (int m=0; m<GENE_SIZE_PREY_NW; m++) {               // weights
            fwrite (&(total_hidden_neuron_populations_prey[p][i][j]->pop[k]->weight[m]),sizeof(double),1,fptr);      
                            }
...
static void parseArgs (int argc, char *argv[],
		       int *numPops, int *popSize, 
		       int *seed, int *netType, bool *analyze,
		       int *seedNet)
{
  ...
  *seed = getpid();
  ...
  // Parse arguments.
  int c;
  while ( (c = getopt_long (argc, argv, "z:n:s:t:h",long_options,
			    &option_index)) != EOF ) {
    switch (c) {
    case 0: if ( !strcmp("stag", long_options[option_index].name) ) {
              STAG = atoi (optarg);
	      printf("STAG : %d\n", STAG);
	    }
    ...
    break; //undocumented
    case 'z': *numPops = atoi (optarg); break;
    case 'n': *popSize = atoi (optarg); break;
    case 's': *seed = atoi (optarg); break;
    case 't': *netType = atoi(optarg); break;
    case 'x': *analyze = true; break; 
    case 'f': *seedNet = optind - 1; break;
    case 'h': 
...
int main(int argc, char *argv[])
{
  ...
  setUpCtrlC();
  parseArgs (argc, argv, &num_hidden_neurons, &popSize, &seed, &netType, &analyze, 
	     &seedNet);
  checkArgs (num_hidden_neurons, popSize);
  reseed(seed);
  PredPrey predprey(num_of_predators, num_of_prey, num_teams_predator, num_teams_prey, prey_move_probability); // this is our experiment.

  cout << "  ESP.C :: number of predator teams:: " << num_teams_predator <<" number of prey teams:: "<<num_teams_prey<< " number of predators in each team:: "<< num_of_predators << " num of prey in each team:: " <<num_of_prey << endl;
  ...
  if (start_generation == 1) {
          Esp esp(num_of_predators, num_of_prey, num_teams_predator, num_teams_prey, num_hidden_neurons, popSize, predprey, netType); 
          ...
          esp.evolve(maxGenerations, num_of_predators, num_of_prey, num_teams_predator, num_teams_prey);       // evolve
  ...
  // --- Load an old ESP from file ---
          ...
          strcat (filename, a);
          strcat (filename, ".bin");
          Esp esp (filename, num_of_predators, num_of_prey, num_teams_predator, num_teams_prey, num_hidden_neurons, popSize, predprey, netType) ;
