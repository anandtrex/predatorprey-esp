...
void subPop::recombine_hall_of_fame(int pred_or_prey_team,
		int pred_or_prey_number, int pop_number) {
	int i = 0;
	for (; i < numBreed - 5; ++i) {
		crossover(pop[i]->weight, pop[findMate(i)]->weight,
				pop[numNeurons - (1 + i * 2)]->weight,
	...
	if (!IS_PREY) {
		for (; i < numBreed; ++i) {
			crossover(
					pop[i]->weight,
					Esp::hall_of_fame_pred[drand48()
							* (Esp::hall_of_fame_pred.size() - 1)][pred_or_prey_team][pred_or_prey_number]->pop[pop_number]->weight,
					pop[numNeurons - (1 + i * 2)]->weight,
					pop[numNeurons - (2 + i * 2)]->weight);
		}
	} else {
		for (; i < numBreed; ++i) {
			crossover(
					pop[i]->weight,
					Esp::hall_of_fame_prey[drand48()
							* (Esp::hall_of_fame_prey.size() - 1)][pred_or_prey_team][pred_or_prey_number]->pop[pop_number]->weight,
					pop[numNeurons - (1 + i * 2)]->weight,
					pop[numNeurons - (2 + i * 2)]->weight);
		}
	}
	...
	if (!IS_PREY) {
		if (IS_COMBINER_NW == 1) {
			for (int i = numBreed * 2; i < numNeurons; ++i)
				if (drand48() < MUT_RATE)
					pop[i]->weight[lrand48() % GENE_SIZE_PRED_COMBINER_NW] +=
							rndCauchy(0.3);
		} else {
			for (int i = numBreed * 2; i < numNeurons; ++i)
				if (drand48() < MUT_RATE)
					pop[i]->weight[lrand48() % GENE_SIZE_PRED_NW] += rndCauchy(
							0.3);
		}
	} else {
		if (IS_COMBINER_NW == 1) {
			for (int i = numBreed * 2; i < numNeurons; ++i)
				if (drand48() < MUT_RATE_PREY)
					pop[i]->weight[lrand48() % GENE_SIZE_PREY_COMBINER_NW] +=
							rndCauchy(0.3);
		} else {
			for (int i = numBreed * 2; i < numNeurons; ++i)
				if (drand48() < MUT_RATE_PREY)
					pop[i]->weight[lrand48() % GENE_SIZE_PREY_NW] += rndCauchy(
							0.3);
		}
	}
}
...
void subPop::crossover(const vector<double> &parent1,
		const vector<double> &parent2, vector<double> &child1,
...
Esp::Esp(int num_of_predators, int num_of_prey, int num_teams_predator,
		int num_teams_prey, int nPops, int nSize, Environment &e,
		int netTp = FF) :
		Envt(e) {
	generation = 0; // start at generation 0

	total_predator_networks_per_team = num_of_predators
			* (num_of_prey * num_teams_prey
					+ num_of_predators * (num_teams_predator - 1))
			+ COMBINE * num_of_predators; //Total Predator Networks in a team

	total_prey_networks_per_team = num_of_prey
			* (num_of_predators * num_teams_predator) + COMBINE * num_of_prey; //Total Prey Networks in a team
	...
	for (int j = 0; j < num_teams_prey; j++) {
		IS_COMBINER_NW = 0;
		for (int i = 0;
				i < total_prey_networks_per_team - COMBINE * num_of_prey; i++) {
			temp_overall_best_teams_prey.push_back(
					genNetType(netType, numPops[0])); // generate new net
	...
	for (int k = 0; k < num_teams_predator; k++) {
		...
		total_hidden_neuron_populations.push_back(
				total_hidden_neuron_populations_per_team);
	}
	...
	for (int k = 0; k < num_teams_prey; k++) {
		...
		total_hidden_neuron_populations_prey.push_back(
				total_hidden_neuron_populations_per_team);
	}
...
Esp::Esp(char* fname, int num_of_predators, int num_of_prey,
		int num_teams_predator, int num_teams_prey, int nPops, int nSize,
...

{
	...
	for (int p = 0; p < num_teams_predator; p++)
		for (int i = 0;
			...
			for (int j = 0; j < numPops[i]; j++)
				for (int k = 0;
						k < total_hidden_neuron_populations[p][i][j]->numNeurons;
						k++)
	...
	for (int p = 0; p < num_teams_prey; p++)
		for (int i = 0;
			...
			for (int j = 0; j < numPops[0]; j++)
				for (int k = 0;
						k
								< total_hidden_neuron_populations_prey[p][i][j]->numNeurons;
						k++)
		...
		for (int p = 0; p < num_teams_predator; p++)
			for (int i = total_predator_networks_per_team
				...
				for (int j = 0; j < numPops[i]; j++)
					for (int k = 0;
							k
									< total_hidden_neuron_populations[p][i][j]->numNeurons;
							k++)
	...
	cout << "Loading from file : each predator is a separate neural net" << endl
			...
			<< total_predator_networks_per_team << endl
			<< "Total Number of Prey Networks:" << total_prey_networks_per_team
			<< endl << "Initial total subpops: "
			<< total_predator_networks_per_team * nPops << endl
			<< "Incremental learning: " << INCREMENTAL_LEARNING << endl
			<< "Burst mutation: " << BURST_MUTATE << endl << "EvalTrials: "
			<< EVALTRIALS << endl << "STAG: " << STAG << endl << endl;
...
void Esp::setupNetDimensions(int num_of_predators, int num_of_prey) {
	...
	case FF:
		GENE_SIZE_PRED_NW = (NUM_INPUTS * num_of_predators)
				/ (total_predator_networks_per_team
						- (COMBINE * num_of_predators)) + NUM_OUTPUTS; // Gene Size per hidden neuron (excepting combiner)
		GENE_SIZE_PREY_NW = (NUM_INPUTS_PREY * num_of_prey)
				/ (total_prey_networks_per_team - (COMBINE * num_of_prey))
				+ NUM_OUTPUTS_PREY;
		GENE_SIZE_PRED_COMBINER_NW = (NUM_INPUTS_PRED_COMBINER
				+ NUM_OUTPUT_PRED_COMBINER); // Gene Size per hidden neuron of a combiner network
		GENE_SIZE_PREY_COMBINER_NW = (NUM_INPUTS_PREY_COMBINER
				+ NUM_OUTPUT_PREY_COMBINER);

		cout << "Gene size per hidden neuron of non-combiner networks: "
				<< " predator:: " << GENE_SIZE_PRED_NW << "  prey:: "
				<< GENE_SIZE_PREY_NW << endl;
		cout << "Gene size per hidden neuron of combiner networks: "
				<< " predator:: " << GENE_SIZE_PRED_COMBINER_NW << "  prey:: "
				<< GENE_SIZE_PREY_COMBINER_NW << endl;
		cout << "total networks in a team  " << " predators:: "
				<< total_predator_networks_per_team << "  prey:: "
				<< total_prey_networks_per_team << endl;
		cout << "FEED FORWARD\n";
		break;
...
void Esp::evolve(int cycles, int num_of_predators, int num_of_prey,
	...
	while (gInterrupt == false && ++generation <= cycles) {
		evalPop(num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey); //evaluate neurons
		...
		for (int k = 0; k < num_teams_predator; k++) {
			for (int i = 0;
				...
				for (int j = 0; j < numPops[i]; j++) {
					...
					total_hidden_neuron_populations[k][i][j]->recombine_hall_of_fame(
							k, i, j); //(Hall of fame probably doesn't work for Competing agents in a single team)
				}
			}
		}
		...
		for (int k = 0; k < num_teams_prey; k++) {
			for (int i = 0;
					i < total_prey_networks_per_team - COMBINE * num_of_prey;
					i++) {
				for (int j = 0; j < numPops[0]; j++) {
					total_hidden_neuron_populations_prey[k][i][j]->qsortNeurons();
					total_hidden_neuron_populations_prey[k][i][j]->recombine_hall_of_fame(
							k, i, j); //(Hall of fame probably doesn't work for Competing agents in a single team)
				}
			}
		}
...
void Esp::evalPop(int num_of_predators, int num_of_prey, int num_teams_predator,
	...
	performEval(num_of_predators, num_of_prey, num_teams_predator,
			num_teams_prey); // evaluate the networks
...
void Esp::performEval(int num_of_predators, int num_of_prey,
		int num_teams_predator, int num_teams_prey) {
	...
	for (int i = 0; i < numTrials; ++i) {
			...
			for (int j = 0; j < total_predator_networks_per_team; j++) {
				for (int k = 0; k < numPops[j]; k++) {
					current_teams[p][j]->setNeuron(
							total_hidden_neuron_populations[p][j][k]->selectNeuron(),
							k);
				}
			...
			for (int j = 0; j < total_prey_networks_per_team; j++) {
				for (int k = 0; k < numPops[0]; k++) {
					current_teams_prey[p][j]->setNeuron(
							total_hidden_neuron_populations_prey[p][j][k]->selectNeuron(),
							k);
				}
		...
		for (int p = 0; p < EVALTRIALS; p++) {
			teams_fitness = Envt.evalNet(current_teams, current_teams_prey,
					generation);
		...
		for (int q = 0; q < num_teams_predator; q++) {
			...
			for (int r = 0; r < num_of_predators; r++) {
				...
				for (int j = 0;
						j
								< (total_predator_networks_per_team
										/ num_of_predators - 1); j++) {
					current_teams[q][r
							* (total_predator_networks_per_team
									/ num_of_predators - 1) + j]->fitness =
							predator_prey_teams_fitness[0][q][r]; // distribute each predator's fitness to constituent networks
					current_teams[q][r
							* (total_predator_networks_per_team
									/ num_of_predators - 1) + j]->addFitness(); // add network fitness to its neurons
				}

				//Following two lines are for the combiner network
				current_teams[q][(total_predator_networks_per_team
						- num_of_predators) + r]->fitness =
						predator_prey_teams_fitness[0][q][r]; // distribute each predator's fitness to constituent networks
				current_teams[q][(total_predator_networks_per_team
						- num_of_predators) + r]->addFitness(); // add network fitness to its neurons
			}
			...
			if (current_teams_sum_individual_fitness[q]
				...
				generation_best_teams_sum_individual_fitness[q] =
						current_teams_sum_individual_fitness[q];
				for (int j = 0; j < total_predator_networks_per_team; j++) {
					generation_best_teams[q][j]->setNetwork(
							current_teams[q][j]);
					generation_best_teams[q][j]->fitness =
							current_teams[q][j]->fitness;
				}
			}
		}
		...
		for (int q = 0; q < num_teams_prey; q++) {
			...
			for (int r = 0; r < num_of_prey; r++) {
				...
				for (int j = 0;
						j < (total_prey_networks_per_team / num_of_prey - 1);
						j++) {
					current_teams_prey[q][r
							* (total_prey_networks_per_team / num_of_prey - 1)
							+ j]->fitness =
				...
				//Following two lines are for the combiner network
				current_teams_prey[q][(total_prey_networks_per_team
						- num_of_prey) + r]->fitness =
						predator_prey_teams_fitness[1][q][r]; // distribute each prey's fitness to constituent networks
				current_teams_prey[q][(total_prey_networks_per_team
						- num_of_prey) + r]->addFitness(); // add network fitness to its neurons
			}
			...
			for (int r = 0; r < num_of_prey; r++) {
				current_teams_prey_sum_individual_fitness[q] =
						current_teams_prey_sum_individual_fitness[q]
								+ current_teams_prey[q][r
										* (total_prey_networks_per_team
												/ num_of_prey - 1)]->fitness;
			}
			...
			if (current_teams_prey_sum_individual_fitness[q]
				...
				generation_best_teams_prey_sum_individual_fitness[q] =
						current_teams_prey_sum_individual_fitness[q];
				for (int j = 0; j < total_prey_networks_per_team; j++) {
					generation_best_teams_prey[q][j]->setNetwork(
							current_teams_prey[q][j]);
					generation_best_teams_prey[q][j]->fitness =
							current_teams_prey[q][j]->fitness;
				}
			}
		}
	...
	for (int q = 0; q < num_teams_predator; q++) {
		...
		if (generation_best_teams_sum_individual_fitness[q]
			...
			overall_best_teams_sum_individual_fitness[q] =
					generation_best_teams_sum_individual_fitness[q];
			for (int j = 0; j < total_predator_networks_per_team; j++) {
				overall_best_teams[q][j]->setNetwork(
						generation_best_teams[q][j]);
				overall_best_teams[q][j]->fitness =
						generation_best_teams[q][j]->fitness;
			}
		}
	}
	...
	for (int q = 0; q < num_teams_prey; q++) {
		...
		if (generation_best_teams_prey_sum_individual_fitness[q]
			...
			overall_best_teams_prey_sum_individual_fitness[q] =
					generation_best_teams_prey_sum_individual_fitness[q];
			for (int j = 0; j < total_prey_networks_per_team; j++) {
				overall_best_teams_prey[q][j]->setNetwork(
						generation_best_teams_prey[q][j]);
				overall_best_teams_prey[q][j]->fitness =
						generation_best_teams_prey[q][j]->fitness;
			}
		}
	}
	...
	//GENERATION BEST
	testNet_result = Envt.testNet(generation_best_teams,
			generation_best_teams_prey, 20);
	...
	//OVERALL BEST
	testNet_result = Envt.testNet(overall_best_teams, overall_best_teams_prey,
			20);

	for (int q = 0; q < num_teams_predator; q++) {
		cout << "\t\tOverall Best Predator team number " << q << " caught "
				<< testNet_result[q][0] << " prey and killed  ";
		cout << testNet_result[q][1] << " predators on average over 20 trials ";
		cout << endl;
	}
		...
		for (int j = 0; j < total_predator_networks_per_team; j++)
			for (int k = 0; k < numPops[j]; k++)
				current_teams[q][j]->setNeuron(
						total_hidden_neuron_populations[q][j][k]->selectNeuron(),
						k);
	}
	// form a random net for predator teams, and test it for 20 trials
	for (int q = 0; q < num_teams_prey; q++) {
		for (int j = 0; j < total_prey_networks_per_team; j++)
			for (int k = 0; k < numPops[0]; k++)
				current_teams_prey[q][j]->setNeuron(
						total_hidden_neuron_populations_prey[q][j][k]->selectNeuron(),
						k);
	}
	...
	for (int q = 0; q < num_teams_predator; q++) {
		cout << "\t\tTypical Predator team number " << q << " caught "
				<< testNet_result[q][0] << " prey and killed  ";
		cout << testNet_result[q][1] << " predators on average over 20 trials ";
		cout << endl;
	}
	...
	if (if_display) {
		...
		for (int p = 0; p < SIMU_TRIALS; p++) {
			if (SHOW) {
				...
				if ((fptr = fopen("pred_prey_location.txt", "at")) == NULL) {
					cout << endl << "Error - cannot open "
							<< "pred_prey_location.txt" << endl;
					exit(1);
				}
				fprintf(fptr, "\n"); //  next line
				fprintf(fptr, "\n"); //  next line
			...
			teams_fitness = Envt.evalNet(generation_best_teams,
					generation_best_teams_prey, generation);
			...
			for (int q = 0; q < num_teams_predator; q++) {
				for (int r = 0; r < num_of_predators; r++) {
					predator_prey_teams_fitness[0][q][r] =
							predator_prey_teams_fitness[0][q][r]
									+ teams_fitness[0][q][r];
					cout << " Generation Best Predator Team number " << q
			...
			for (int q = 0; q < num_teams_prey; q++) {
				for (int r = 0; r < num_of_prey; r++) {
					predator_prey_teams_fitness[1][q][r] =
							predator_prey_teams_fitness[1][q][r]
									+ teams_fitness[1][q][r];
					cout << " Generation Best Prey Team number " << q
							<< "  Prey Number " << r << "  fitness:: "
							<< teams_fitness[1][q][r] << endl;
				}
			}
		}
		...
		for (int q = 0; q < num_teams_predator; q++) {
			for (int r = 0; r < num_of_predators; r++) {
				cout << "Average fitness for Predator Team Number " << q
						<< " Predator Number " << r << ":: "
						<< predator_prey_teams_fitness[0][q][r] / SIMU_TRIALS
						<< endl;
			}
		}
		for (int q = 0; q < num_teams_prey; q++) {
			for (int r = 0; r < num_of_prey; r++) {
				cout << "Average fitness for Prey Team Number " << q
						<< " Prey Number " << r << ":: "
						<< predator_prey_teams_fitness[1][q][r] / SIMU_TRIALS
						<< endl;
			}
		}
		SHOW = 0;
	...
	if (generation == 5)
		save("out_5.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 100)
		save("out_100.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 200)
		save("out_200.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 300)
		save("out_300.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 500)
		save("out_500.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 1000)
		save("out_1000.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 1500)
		save("out_1500.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 2000)
		save("out_2000.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 2100)
		save("out_2100.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 2200)
		save("out_2200.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 2300)
		save("out_2300.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 2400)
		save("out_2400.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 2500)
		save("out_2500.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 3000)
		save("out_3000.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 5000)
		save("out_5000.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 7000)
		save("out_7000.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 9000)
		save("out_9000.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 9100)
		save("out_9100.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 9200)
		save("out_9200.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 9300)
		save("out_9300.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 9400)
		save("out_9400.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
	else if (generation == 9500)
		save("out_9500.bin", num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey);
...
void Esp::save(char* fname, int num_of_predators, int num_of_prey,
		int num_teams_predator, int num_teams_prey) {
	FILE* fptr;
...
int Esp::removeSubPop(vector<Network*>& team, int pred, int num_of_predators,
		int num_of_prey) {
	int sp;
...
static void parseArgs(int argc, char *argv[], int *numPops, int *popSize,
		int *seed, int *netType, bool *analyze, int *seedNet) {
	...
	*seed = getpid();
	...
	//  {"plot", 0, &PLOTTING, 1},
	//  {"delta", 0, &DELTA, 1},
			{ "minimize", 0, &MIN, 1 }, { "stag", 1, 0, 0 }, { "mutation", 1, 0,
					0 } };
	...
	while ((c = getopt_long(argc, argv, "z:n:s:t:h", long_options,
		...
		case 'h':
			...
			cout
					<< "         0 = Feedforward, 1 = SRN, 2 = 2ndOrder Recurrent\n";
			cout << "         3 = Fully Recurrent\n";
			//cout << "  -f F : Seed network. \n";
			cout << "  --mutation M : Mutation rate in percent (0,1). \n";
			cout
					<< "  --stag STAG : Stagnation threshold, number of generations. \n";
			cout << "  --minimize : Minimize fitness. \n";
			//      cout << "  -x   : analyze nets.\n";
			cout << "  -h   : this message.\n";
			cout << "Default: " << argv[0] << " -z " << DEFAULT_NUM_POPS
					<< " -n " << DEFAULT_POP_SIZE << " -s getpid()" << " -t "
					<< FF
...
//--------------------------------------------------------------------
static int sAttribList[] = { GLX_RGBA, GLX_RED_SIZE, 1, GLX_GREEN_SIZE, 1,
...
int main(int argc, char *argv[]) {
	...
	setUpCtrlC();
	parseArgs(argc, argv, &num_hidden_neurons, &popSize, &seed, &netType,
			&analyze, &seedNet);
	...
	cout << "  ESP.C :: number of predator teams:: " << num_teams_predator
			...
			<< " num of prey in each team:: " << num_of_prey << endl;
	...
	if (if_display) {
		...
		win = XCreateWindow(dpy, RootWindow(dpy, vi->screen), 30, 30, 500, 500,
	...
	if (start_generation == 1) {
		Esp esp(num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey, num_hidden_neurons, popSize, predprey, netType);
		...
		esp.evolve(maxGenerations, num_of_predators, num_of_prey,
				num_teams_predator, num_teams_prey); // evolve
	...
	// --- Load an old ESP from file ---
		...
		strcat(filename, a);
		strcat(filename, ".bin");
		Esp esp(filename, num_of_predators, num_of_prey, num_teams_predator,
				num_teams_prey, num_hidden_neurons, popSize, predprey, netType);
