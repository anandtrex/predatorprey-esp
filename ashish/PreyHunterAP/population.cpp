...
bool Population::spawn(Genome *g,int popsize) {
	...
	//Create size copies of the Genome
	//Start with perturbed linkweights
	for(count=1;count<=popsize;count++) {
		//cout<<"CREATING ORGANISM "<<count<<endl;
...
bool Population::speciate() {
	...
	int counter=0, i = 0; //Species counter
...
bool Population::epoch(int generation) {
	...
	*/
	...
	if (((*(((*curspecies)->organisms).begin()))->orig_fitness)>
        ...
        //Print overall best fitness to file
        std::ofstream outfile("results_new_weights", std::ios::app);

        outfile<<highest_fitness<<std::endl;
