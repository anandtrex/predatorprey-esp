...
namespace PredatorPreyHunter {
  ...
  int PredatorSingleOutput::computeHigher( NEAT::Network* ptrNetworkCurrent, const std::vector<AgentInformation>& vAgentInformation ) {
  ...
  int PredatorSingleOutput::computePrey( NEAT::Network* ptrNetwork, const std::vector<AgentInformation>& vAgentInformation ) {
    // prepare inputs for network
    // assuming node order
    // 1 bias node
    // 2 relative prey position x
    ...
    if ( !foundPrey ) {
      cerr << "PredatorSingleOutput::computePrey() I could not find the prey" << endl;
      throw 1; // throw something meaningful later
    }
    // borrowed from Aditya and Padmini
    int x_dist, y_dist;
  ...
  int PredatorSingleOutput::computeHunter( NEAT::Network* ptrNetwork, const std::vector<AgentInformation>& vAgentInformation ) {
    // prepare inputs for network
    // assuming node order
    // 1 bias node
