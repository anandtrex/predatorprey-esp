...
namespace PredatorPreyHunter {
  ...
  double Domain::calculateFitness( const Caught& caught, const uint& stepCurrent ) {
    ...
    if ( PREY_CAUGHT == caught ) { // Yay 
      ...
      fitness = static_cast<double>(distancePrey);
      return fitness;
    }
  }
  ...
  double Domain::run( string pathFile ) {
    ...
    uint noSteps = 0;
    Caught caught; // for catching caught signal
    Position positionPredator, positionPrey;
    while ( noSteps < maxSteps ) {
      //cout << "step: " << noSteps << endl;
      ...
      fout << -10 << " " << -10 << endl; // negative values imply that the hunter will not be drawn by the visualizer
      caught = step(); 
      switch( caught ) {
        case PREY_CAUGHT:
          // if prey is caught
